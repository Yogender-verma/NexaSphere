...
export class FinancialService {
  ...
  async createBudget(budgetData, user) {
    ...
    const budget = { ...budgetData, createdBy: user.id };
  ...
  async getBudgets(user) {
    ...
    if (user.role === 'organizer') return allBudgets.filter((b) => b.createdBy === user.id);
    return allBudgets;
  }
  ...
  async createExpense(expenseData, user) {
    ...
    const expense = { ...expenseData, status: 'submitted', submittedBy: user.id };
  ...
  async getExpenses(budgetId, user) {
    ...
    if (user.role === 'organizer') return expenses.filter((e) => e.submittedBy === user.id);
    return expenses;
  }
  ...
  async updateExpense(id, patch, user) {
    ...
    if (patch.status && patch.status !== expense.status) {
      ...
      if (patch.status === 'approved' || patch.status === 'reimbursed') patch.approvedBy = user.id;
    }
  ...
  async createRevenue(revenueData, user) {
    ...
    const revenue = { ...revenueData, createdBy: user.id };
  ...
  async getRevenues(budgetId, user) {
    ...
    if (user.role === 'organizer') return revenues.filter((r) => r.createdBy === user.id);
    return revenues;
  }

  async deleteRevenue(id, user) {
    const revenue = await financialRepository.getRevenueById(id);
    if (!revenue) throw new Error('Revenue entry not found');
  ...
  async getBudgetVariance(budgetId, user) {
    ...
    const comparisons = categories.map((category) => {
      ...
      return {
        ...
        status: budgeted - actual >= 0 ? 'under_budget' : 'over_budget',
      };
    });
